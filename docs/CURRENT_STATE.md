...
| Marketing sections                | Reusable components under `src/components/sections/` and `src/components/marketing/`. Public **site header is sticky**. Home uses **alternating canvas / sand-deep bands** with generous `py-section`. Home hero has a short CSS entrance; five homepage sections fade up on scroll. Photos use `TokenImage` (`next/image` `placeholder="blur"` + sand-deep reserved slot; shimmer off for reduced motion).  |
