...
| Design system                     | Tokens in `globals.css` with **dark-mode canvas remap** (`html.dark`, mangrove-deep + gold-mustard, not a second grey scale); layout + primitives; sun/moon toggle in the sticky header; rendered at **`/style-guide`** (`noindex`)                                                                                                                                                                          |
| Public pages                      | Home (full section stack), `/rooms`, `/rooms/deluxe-ac-room`, `/gallery`, `/about`, `/location`, `/contact`, Privacy, Terms                                                                                                                                                                                                                                                                                  |
| Marketing sections                | Reusable components under `src/components/sections/` and `src/components/marketing/`. Public **site header is sticky** (`top-0 z-50`) with a compact theme toggle. Home hero has a short CSS entrance; five homepage sections fade up on scroll (`prefers-reduced-motion` respected).                                                                                                                        |
